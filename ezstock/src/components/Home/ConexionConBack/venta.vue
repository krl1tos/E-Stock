<template>
  ...
  <template v-slot:activator="{ props }">
      <v-btn v-bind:="props" elevation="0" variant="tonal" color="blue" prepend-icon="fa-solid fa-truck-fast">Venta</v-btn>        
  </template>
  <v-card>
      <v-card>
            <v-card-title class="text-h5 text-center" id="titulo"> Formulario de Venta</v-card-title>
...
productos.forEach(producto => {
  const productoActual = producto.nombre_producto
  listaProductosNombre.push(productoActual)
})
